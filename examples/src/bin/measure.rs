...
use examples::bench::{prove, sha256_no_lookup_prepare, sha256_with_lookup_prepare};
use jemalloc_ctl::{
	epoch,
	stats::{self},
};
...
fn main() -> Result<(), Error> {
	...
	println!("Constraint system size (no lookup): {} KB", len as f32 / 1024.0);
	...
	constraint_system
		...
		.unwrap();
	let len = write_buf.len();
	println!("Constraint system size (with lookup): {} KB", len as f32 / 1024.0);
